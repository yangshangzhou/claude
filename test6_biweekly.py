...
class BiweeklyCheckTest(QCAlgorithm):
    """
    ============================================================================
    R2-测试3: 双周调仓版 —— 同Test5权重+阈值，但每两周调一次
    ============================================================================

    验证问题: "提高调仓频率，能不能更快捕捉市场转向？"

    参数: 同 Test5 (Trend 0.40, Rate 0.25, Curve 0.20, PMI 0.15, BULL>0.10)
    唯一区别: 每月调仓 → 每双周调仓

    对比:
    - Test5 (月频+新权重): 待定
    - 如果本版本 > Test5 → 频率提升有效，月频确实太慢
    - 如果本版本 ≈ Test5 → 频率不是瓶颈，问题在信号本身
    ============================================================================
    """
    ...
    def initialize(self):
        ...
        self.schedule.on(
            self.date_rules.month_start(15),  # 每月第15天
            self.time_rules.after_market_open("SPY", 90),
            self.biweekly_check
        )
